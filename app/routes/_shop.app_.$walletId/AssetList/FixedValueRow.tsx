...
export type FixedValueRowProps = {
  asset: AssetWithPrice
}

export const FixedValueRow = memo(({ asset }: FixedValueRowProps) => {
  const fetcher = useFetcher({ key: asset.id })
  ...
  return (
    ...
    >
      <fetcher.Form className="hidden justify-center @md:flex " method="DELETE">
        <button type="submit" name="assetId" value={asset.id}>
          {fetcher.state === 'idle' ? (
...
function AmountInput({ asset }: { asset: AssetWithPrice }) {
  const fetcher = useFetcher({ key: asset.id + 'PATCH_amount' })
...
function PriceInput({ asset }: { asset: AssetWithPrice }) {
  const fetcher = useFetcher({ key: asset.id + 'PATCH_price' })

  const [value, setValue] = useState(asset.price)

  const changed = value !== asset.price
  ...
  return (
    <fetcher.Form className="relative min-w-20" method="PATCH">
      ...
      <input
        ...
        value={brl(value)}
        onChange={(e) => {
          const val = currencyToNumber(e.target.value)
          if (!isNaN(val)) setValue(val)
        }}
        onFocus={(e) => e.target.select()}
        name="price"
