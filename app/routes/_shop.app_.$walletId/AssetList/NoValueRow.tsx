...
import { DomainAsset } from '~/services/assetService/index.server'
...
export type NoValueRowProps = {
  asset: DomainAsset
}

export const NoValueRow = memo(({ asset: stock }: NoValueRowProps) => {
  ...
  return (
    ...
    >
      ...
      <span className="col-span-2 flex items-end gap-4 @md:hidden">
        <EasyTooltip label="Preço atual">R$ --,--</EasyTooltip>
      ...
      <EasyTooltip label="Valor total">
        <span className="hidden text-sm @md:inline">R$ --,--</span>
      </EasyTooltip>
...
function AmountInput({ asset }: { asset: DomainAsset }) {
